...
規約2にはもう一段ある。`PUBLISHED_TABLES` 自体はまだテスト側の宣言なので、production に表や列を足してこちらに足さなければ、同じ形で漏れる。そこで**宣言と実出力の一致**そのものを検査する: 生成された dashboard に出ている `###` 見出しの集合が宣言と一致すること、各表の列数が宣言した価格ペアの数と一致すること。renderer に宣言外の表や列を足すと落ちる。
