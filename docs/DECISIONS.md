...
- **dashboard は結論から始める。** `## 仮説検証` の直下に「827件の比較をBenjamini-Hochbergで補正した結果、統計的に主張できる項目は0件。以下の表は記述であって、検証を通過した所見ではない。」を出す。件数と残存数は集計結果から導出するので、何かが残れば文面も変わる。
...
未対応として残るもの（変わらず別PR）: `knowledge.py` に留保・寄り付き起点・多重比較補正が入っていないこと、そこから凍結された19仮説のうち8件が汚染された組合せに乗っていること、`outputs/` の移行成果物が再生成できないこと。無効化は削除ではなく、新しいレジストリversionとして積む形で行う。現在の緩和検査は仮説の削除を拒否するので、「無効化は削除ではない」を検査が区別できるようにする設計が先に要る。
