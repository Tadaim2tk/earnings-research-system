...
Consequences: monitoring pipelineが起動しさえすれば、bundleが作れない障害でもHumanに届く。workflow自体が起動しない場合（Actions停止、schedule無効化）は依然としてstale閾値が唯一の検知経路であり、これは残る制約である。observation、robots、append-only、pending、stale閾値、承認gateは変更しない。
