...
**27.7% が引け後ではなかった。** 引け後 170（72.3%）/ 後場 41（17.4%）/ 昼休み 21（8.9%）/ 前場・寄り前 3（1.3%）。**65件は当日の終値がすでに反応を含んでいる。**〔訂正 2026-08-30: 初出は「28%・66件・昼休み22」。Amendment が matched を 236→235 に減らしたときに内訳だけ再計算しておらず、分母236のままだった。落ちた1件は `80310_dup` が借りていた三井物産 2026-08-04 12:00（昼休み）である。〕「終値で判断して翌日寄り付き」はその66件で成立しない。以前 i0優勢26件から間接的に疑っていたものが、実測になった。
...
Context: ERS-ADR-0065 で分かったとおり、`www.release.tdnet.info` は文書を約31日で落とす。索引は2019年まで遡れるのに本文は残らないので、**後から集めることはできない**。254件の台帳のうち **132件は気づいた時点で既に失われていた**——28の決算日にまたがるので**決算日あたり4.7件、暦日あたり2.7件**である。〔訂正 2026-08-30: 初出は「1日あたり約8件」。それは台帳の最も多い日の社数であって、喪失の平均ではない。〕

索引の目録は 2021-01-01〜2026-08-25 で **1,473日・短信101,803件**が揃った。それに対して本文は **103件**しか無い。〔訂正 2026-08-30: 初出は102件。ERS-ADR-0065 と 0068 は同じ数を103件と書いており、ここだけ1件少なかった。〕この比が、31日の窓が意味することの全部である。
...
- **既定は台帳にある銘柄だけ。** 全短信は台帳の決算日で**中央値24本・平均95本・最大750本**（2026-08-07）、台帳側の平均5.6件/日に対して**平均17倍・ピーク83倍**になる。`--all` を明示しない限り広げない。〔訂正 2026-08-30: 初出は「1日250本で30倍」。保存済みの索引で数え直すとどの集計とも一致しなかった。結論（桁で増える）は変わらない。〕
...
**⚠ まだスケジュールしていない。** `.github/workflows` にも launchd にも、このツールを呼ぶものは無い。**書いただけでは1件も確保されない。**〔2026-08-30 に ERS-ADR-0070 で決着。launchd で毎日22時、専用 clone は `~/.ers-corpus/repo`。〕 どこで走らせるか（コーパスがローカルにあるので CI からは書けない）は未決である。
...
Consequences: `INSTRUMENT_VERSION = 5f986834218c05b5`。テスト 1208→1218。確保済み103件のうち **84件（82%）で本文を取得**でき、残り19件は `no_section` として記録される。本文の長さは中央値 2133.5字（84件の中央2値は 2132 と 2135）。
...
そして実際に判断できた。上限6で落ちた**6件**を見ると、**5件**は 7〜10 個の**別々の**逆風・追い風（中東情勢／物価上昇／インフレ再燃…）を挙げた正当な列挙で、暴走は1件だけだった——4755 の「黒字化」が3回（7件のうち相異なりは5件）。〔訂正 2026-08-30: 初出は「5件・4件・8〜10個」。同じ ADR が「抽出できた文書 77→83（+6）」と書いており、内部でも6件のはずだった。〕**反復を潰すのは重複排除の仕事で、件数上限の仕事ではない。** 上限だけで両方を捌こうとすると、正当な列挙を捨てるか暴走を通すかになる。重複を先に落とし、上限を10へ緩めた（v4 `3d1926f1`）。
...
- **境界が `YYYY-MM-DD` でなければ `MalformedWindow` を上げる。** 黙って別の日を測らせない。
- **`first >= last` を `None` にする。** 1点から区間の騰落は出ない。
- **ADR の数値を訂正する。** 該当箇所に〔訂正 2026-08-30〕として、初出の値・正しい値・なぜ間違えたかを残す。消さない。
...
Consequences: テスト 1240→1243。保存済みスナップショットで測り直し、ADR-0066 の数値（銀 +19.02% / 銅 +3.00%、探索期 銀 −10.85% / 銅 +2.99%）が**変わらない**ことを確認した。
