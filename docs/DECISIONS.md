...
Note: 起案時は0074として書いたが、同番号を PR #56（段階評価）が先に main へ入れた
ため、内容を変えずに0077へ移した。0075は PR #78、0076は PR #79 が使う。**採番の
衝突を避けるために番号を飛ばしており、連番であることより一意であることを採る。**
...
Approval: `tools/build_disclosure_index.py` が索引を日ごとに丸ごと保存する形を
取るにあたり、`AGENTS.md` の Required Conduct（外部からの収集・第三者コンテンツの
保存を加える前に取得元の条件を確認する）に従って確認した結果を記録する。**確認を
先に済ませていなかったのは手順違反で、レビューの指摘で判明した。**

Note: 採番は0079。0074は PR #56 が main へ入れた分、0075は PR #78、0076は PR #79
での #56分の採番し直し、0077は本PR、0078は PR #79 の新方針。

Context: `~/.ers-corpus/tdnet/` は決算短信しか持っていなかった。前の収集が
`is_tanshin` で漉していたためで、そのせいで「増資・TOB・単元変更は原理的に判定
できない」という誤った結論を書いた。実測で 2026-08-28 の索引は227件のうち短信は
5件で、**97%を捨てていた**。全幅で保存する形に変えたが、そこで保存対象が
「短信の表題」から「全ての適時開示の表題とURL」へ広がった。

取得元は `webapi.yanoshin.jp`。確認した内容は次のとおり。

- `robots.txt` は `User-Agent: *` に対し `Allow: /`
- `llms.txt` を公開しており、**認証不要**、明示のレート制限は無く
  「頻度は常識的に」とのみ述べる
- **保存・保持・再配布については何も述べていない**
- 商用利用・研究利用の制限も述べていない
- 帰属表示の要求も述べていない

つまり**保存を明示的に許してもいないし、禁じてもいない**。

Decision: 明示の許可が無い以上、次の範囲に留める。

1. 保存先は `~/.ers-corpus/` とし、**リポジトリには入れない**。このリポジトリは
   public であり、表題やURLをそのまま公開する形にはしない。リポジトリ側は
   manifest と digest だけを持つ。
2. 取得の頻度は1日あたり1リクエスト、間隔0.8秒とする（`llms.txt` の「常識的に」
   に対する具体化）。既に在る日は取りに行かない。
3. **再配布しない。** 索引から導いた集計（種別ごとの件数など）は出すが、表題の
   一覧を成果物として出さない。
4. 取得元が条件を明示した場合、または保存を禁じる旨が示された場合は、この判断を
   見直して `~/.ers-corpus/tdnet_full/` を破棄する。

Consequences: 会社の行為105,624件を索引から導けるようになり、上場廃止・TOB・
増資を「原理的に判定できない」とした誤りが解消した。一方、保存の可否は取得元が
述べていない以上こちらの判断であり、**許可を得たわけではない**。この曖昧さを
消さずに記録しておく。
## ERS-ADR-0075
...
Note: 番号は0075から始める。0074は未merge の PR #56（段階評価）が使っている。
番号を詰めるより、確定済みの番号と衝突させない方を採った。

Context: 市場をまたぐ集計と、価格の欠測の扱いで、**動いているのに逆の答えを返す**
経路が2つ見つかった。

第一に、`pandas.pct_change()` は既定で欠測を前の値で埋める（2.0系まで
`fill_method="pad"`）。20系列を1つの表に並べると、週末も動く仮想通貨のせいで
日付が1,382日から2,066日に膨らみ、日本市場が閉まっている684日は日経の欄が空欄に
なる。そこを埋めると、**閉まっていた681日が「騰落ちょうど0.00%」の実在する観測に
化ける**。日経とS&Pの相関は2022年で0.67が0.46に薄まり、直近60日の位置づけが
「5年の下から64%（中央値より上）」と出た。正しく組むと「下から28%（中央値より
下）」で、報告した結論の符号が逆だった。

空欄を埋めないだけでは足りない。週末の行を残したまま1日ずらすと月曜の日経が
「日曜の米国」と組まれて落ちる。さらに窓が1日ずれており、日本の営業日 D と前の
営業日 P に対して拾うべき米国の立会は `[P, D)` なのに `(P, D)` にしていた。米国の
「P日の立会」は日本のP日の引けより後に終わるので、D日にとっては新しい情報である。
この1日で連続する営業日を全部捨てており、BTCの下げ／上げの傾き比が14倍と出た
（正しくは2.4倍）。

第二に、価格が取れないことと、株価が存在しないことを分けていなかった。バッチ取得
に失敗した336銘柄を上場廃止として扱いかけたが、**118銘柄は2026年も決算を出して
いる現役の会社**で、個別に取り直せば取れた。逆に、上場廃止で系列が終わった銘柄を
落とすと、残るのは生き残った会社だけになりリターンが上振れする。

Decision: `regime/align.py` と `prices/coverage.py` を追加する。

`align.py` は系列ごとに**自分の営業日だけで**騰落を作り、`merge_asof` 相当の
後方as-of結合で、日本の各営業日に**その日より前で直近**の相手の立会を当てる。
同じ日付は拾わない（拾うと未来を見たことになる）。`padded_zero_days` は、暦で
並べたときに偽の0.00%が何日ぶん生まれるかを数える——直さずに使ってしまったときの
被害を測るために置く。**pandas に依存しない**。CIは `.[dev]` しか入れないので、
依存すると試験が収集ごと落ちるか、任意依存にしてCIで一度も走らない試験になる。
飛ばされる試験は何も守らない。

`coverage.py` は欠測の理由を語彙として固定する。
...
「出口の前に系列が終わった」は最終日との比較では出せない。出口はセッション番号で
決まるので、必要な本数が残っているかを数える。日付は辞書順で比べる前に
`date.fromisoformat` と正準表記の一致で検査する——`\d` は全角数字を通し、
`"2026-8-28"` は `"2026-08-28"` より後ろに並ぶ。
...
Approval: ICECO監視が8月28日から `stopped` のまま6日間止まっていた件の原因を割り、
抽出を直す作業。監視の再開そのもの（gap_acknowledgement）は
`requires_human_decision: true` が立っており、本ADRの範囲外。

Note: 番号は0080。0079までは既に main にある。

Context: 8月28日に `ICECO_EARNINGS_CALENDAR` が `state_unavailable: stale gap
exceeded 60-hour normal threshold` で `stopped` に落ち、Issue #58 が開いたまま
6日間放置されていた。**ワークフローは success を返し続けていた** ——
`monitor` ジョブが `plan` の空の一覧で skipped になるためで、
「workflowがsuccessなら観測している」は誤りという、既に記録済みの罠を
気づく側が踏んだ形である。

原因は取得元の変更だった。アイスコのIRカレンダーが **日付を落として月だけの表**
になっていた。ページ自体は生きている（HTTP 200・0.2秒・robotsも許可）が、
`2026年11月13日 2027年3月期第2四半期決算発表` という「日付＋ラベル」の行が消え、
「年間スケジュール」の格子に変わった。`_SCHEDULE_DATE` は `20XX年X月X日` を
必須にしているので、構造上どうやっても一致しない。

**仕組みは正しく働いた。** 読めなくなったときに `no_change` と報告せず、
`stopped` に落として人の判断を求めた。黙って「変更なし」を積み上げていれば、
決算発表日の変更を見落としたまま「監視できている」と読めていた。

Decision:

- **月だけの行を、日付を作らずに読む。** `monthly_schedule` を新設し
  `8月=第１四半期決算発表` の形で持つ。`earnings_schedule`（正確な日付）、
  `approximate_schedule`（年月＋上旬中旬下旬）とは**別の欄**にする。
  欄が分かれていること自体が、公表された粒度を示す。**無い精度を足さない。**
- **見出し行と、月とラベルが交互に並ぶ側を、続く月の数で分ける。** 見出し行は
  12か月が続けて並び、発表内容は別の行にある。並び順で対応を取ると
  **最後の3月に次の行の先頭の決算発表が付く**。続く月が3つ以上なら見出し行と
  みなして対象から外す。クラス名には依存しない——装飾の変更で壊れるため。
- **日付も月も取れない形になったら、従来どおり落とす。** 読めないものを
  黙って通さない。

Consequences: テスト 1342→1346。実際に読めなくなった構造をそのまま固定物にした
ので、次に取得元が作りを変えたら落ちる。ラベルは全角のまま持つ（`第１四半期`）
——表記の変更も変更として拾うため。

**監視はまだ止まったままである。** 抽出が直っても `stopped` は自動で解けない
（設計どおり）。復帰には `gap_acknowledgement` の投入が要り、これは
`requires_human_decision: true`。**抽出を直さずに復帰だけさせると60時間後に
また止まる**ので、順序はこのADR→復帰である。


### ERS-ADR-0080 Amendment (2026-09-06)

Codex の指摘（P1）。**日付の行が1行でもあれば月だけの行を見ない書き方にしていた。**

```python
monthly = _monthly_schedule(segments) if not exact and not approximate else []
```

取得元が日付を落とすのは一度に全部とは限らない。近い四半期だけ日を出し、先の
四半期は月だけ、という混在は自然な途中経過である。その形で「2月＝第３四半期
決算発表」が3月へ動いても、**指紋は変わらず `no_change` と報告される。**
落ちてくれない分だけ、読めなくなって停止するより質が悪い。
本ADRが「読めないものを黙って通さない」と書いた、その裏側を残していた。

実際に確かめた。混在した表で月だけの行を 2月→3月 へ動かし、
`stable_metadata` が完全に一致することを見た。

条件を外し、月だけの読み取りを常に走らせる。日付だけの表から拾ってしまう心配は
実測で無い——`^N月$` に一致するのは月だけを置いた行で、`2026年11月13日 …` は
一致せず、12か月が続けて並ぶ見出しは続く月の数で外れる。

指摘はもう一点、**混在の要約テストが metadata を手で組み立てるだけで、パーサの
経路を通っていない**ことも突いていた。そのとおりで、パーサから通す試験を3本足した。

テスト 1348→1351。

さらに Codex の P2 指摘。**日付が節に割れているだけの行を、月だけの行と読んでいた。**

`<span>2026年</span><span>11月</span><span>13日</span>` は `_SCHEDULE_DATE` の
`(20\d{2})年(\d{1,2})月(\d{1,2})日` に一致しないので月だけの側へ落ちる。再現した:

```
earnings_schedule      none
monthly_schedule       11月=13日2027年3月期第2四半期決算発表
```

**日付は公表されているのに「日付は無い」と報告し、ラベルにも日付の断片が残る。**
観測は成功するので、誰も気づかない。月だけの経路を常に走らせるようにした本
Amendment が、混在ページでこの形にも当たるようにしていた。

月の隣に年や日の断片が並んでいれば、それは月だけの行ではなく割れた日付である。
検出したら落とす。**取り違えた読み方で観測を成功させない**——本ADRの原則の側。
再構成はしない。節の並びから日付を組み直すのは、構造を推測することになる。

年の見出しの下に12か月の格子が並ぶ形は誤検知しない。続く月の数で見出し行が
先に外れるため、この検査までは来ない。実測で確認した。

テスト 1351→1354。

続けて Codex の P2 指摘。**続く月が3つの並びを、見出し行として捨てていた。**

`_MONTH_RUN_LIMIT = 3` は「続く月が3つ以上なら見出し行」という規則だった。
しかし**四半期は3か月ある**。発表が四半期の3番目の月にあると
`4月 5月 6月 決算発表` の並びになり、その3つが丸ごと消える。再現した:

```
発表が5月    : 5月=決算発表;8月=…;11月=…;2月=…
発表を6月へ  : 8月=…;11月=…;2月=…      ← 5月が消え、6月は出ない
```

**通知には削除だけが載り、新しく公表された月は出ない。** 上限を4にして、
続く月が3つまではデータ行として読む。見出し行の12か月は今までどおり外れ、
最後の3月が次の行のラベルを拾うこともない。

**ただしこれは長さという代用で見出しかどうかを決めている。** ラベルの無い
四半期が2つ続けば月は6つ並び、その並びは今も落ちる。段の折り返し方が変われば
同じことが起きる。節の並びだけからは、見出しの最後の月とラベルを持つ月を
見分けられない。**残った穴として、ここに書いておく。**

テスト 1354→1356。

さらに2件。**割れた日付の検査が広すぎ、残った穴の記録先が違っていた。**

一つ目（P2）。割れた日付の検査を、発表の行かどうかを見る前に置いていた。
月だけの読み取りを常に走らせるようにした分、**ページのどこかに無関係な
`2026年` `11月` の並び（アーカイブのナビゲーション等）があるだけで、
正しく読めているカレンダーが `parse_error` で止まる。** 再現した:

```
元のカレンダー          : SourceObservation
無関係なナビを足した後  : ObservationFailure parse_error
```

検査を、ラベルに `決算発表` があると分かった後へ移す。割れた日付の側は
ラベルを持つので落ち方は変わらない。

二つ目（P1）。残った穴を ADR にだけ書いていた。`AGENTS.md` は
「不確かな前提は `docs/OPEN_QUESTIONS.md` に記録する」と定めている。
**決定の文書に埋めると、決まったことのように読める。** 見出し行を長さで
見分けている件を OPEN_QUESTIONS へ移した。
...
Note: 番号は0081。0080は PR #80 で起案済み。

Context: ERS-ADR-0080 は8月26日の取得元の変更を割り出したが、**なぜ8月28日以降
一度も再試行されなかったかを説明していない**。理由は別にあった。

`registry.py::_is_due` の通常日の判定は `17 <= local.hour < 21` である。cronは
6枠あるが、この窓に入るのは **17:17 JST の1枠だけ**。設計上は3時間44分の遅れを
吸収する余裕を持たせてあったが、余裕は遅延に対するもので、**枠が配信されない
ことに対する備えは無かった**。

2026-08-29以降、GitHubは6枠のうち4枠しか出さなくなった。落ちた2枠は
00:17 UTC と **08:17 UTC（= 17:17 JST）**。実測（run一覧）:

```
9/1(月) 01:11 06:29 14:16 22:31 JST — 窓に入った実行なし
9/2(水) 04:25 07:37 13:41 21:48 JST — なし
9/3(木) 04:24 07:40 13:38 21:51 JST — なし
9/4(金) 04:18 07:37 13:39 21:45 JST — なし
```

**5営業日連続で誰も観測せず、workflow は success を返し続けた。** plan が空だと
monitor ジョブが skipped になるためで、ERS-ADR-0080 が記録した罠と同じである。

生き残っていた 21:17 JST の枠は毎日30分ほど遅れて 21:45 に着地し、窓の締切
21:00 を外していた。**枠は動いていたのに、時計の条件が拾わなかった。**

Decision:

- **通常日は「今日もう成功したか」で決める。** 対象自身の checkpoint の
  `last_success_at` を JST の暦日に直し、今日と同じなら due にしない。
  まだなら大引け後（`hour >= 17`）のどの枠でも走る。17:17 が落ちても 21:17 が拾う。
  **一日一度は時計ではなく事実で守る。**
- **状態が読めないときだけ従来の窓に戻す。** 空文字と、naive・壊れた文字列を
  同じ扱いにする——どちらも「どの日に観測できたか」を言っていない。
  読めない状態で無制限に再試行すると要求の頻度が上がり、
  `system_policy:public-web-low-frequency-v1` の前提を崩すため、
  上限のない再試行にはしない。
- **plan ジョブは有効な全 target の state を取る。** これまで schedule source
  だけを取っていた。判定に使う以上、対象が揃っていなければ全員が時計に戻る。
- **時計に戻ったことを黙らせない。** 状態が読めなかった対象を stderr に出す。
  既存の schedule source の fallback が同じ理由で通知していたのに、こちらだけ
  黙っていた。**silent fallback は今回の停止そのものの形である** ——
  観測されないまま5営業日が消え、workflow は success を返し続けた。
  戻ること自体は正しい。黙って戻ることが正しくない。
- **`--schedule-state` を `--previous-state` に改名した。** 同じディレクトリが
  発表日と最終成功の両方を運ぶようになり、`schedule` という名前が中身より
  狭くなったため。テストの補助関数 `write_schedule_state` も同様に改名した。

Consequences: テスト 1348→1364。実際の停止日（2026-09-02 21:45 JST）の
checkpoint を入力に、直す前は `[]`、直した後は2 target とも計画されることを
確認した。同じ日の2度目は `[]` に戻り、翌営業日にまた計画される。

**イベント窓と決算発表日の6枠制は変えていない。** 一日一度の規則は通常日のものである。

**これは復帰そのものではない。** 監視は `stopped` のままで、
`gap_acknowledgement` の投入が別に要る。本ADRが直すのは、
復帰したあと**また5営業日黙って止まらない**ようにする部分である。

### ERS-ADR-0081 Amendment (2026-09-06)

Codex の指摘（P2）。**並びで待たされた2本目が、同じ日に二度取りに行く。**

plan は同時に走っている別の実行の結果を見られない。遅れて重なった2本が
どちらも「今日はまだ成功していない」を見ると、両方が due になる。target ごとの
concurrency は順番に並べるだけなので（`cancel-in-progress: false`）、
**2本目は更新後の状態を取得しておきながら、そのまま観測する。**

この競合は本ADR以前からあったが、**本ADRが窓を広げた。** 以前の
`17 <= hour < 21` では 17:17 の枠しか入らず、21:17 の枠（実際は21:45頃に着地）は
外れていた。`hour >= 17` にしたことで両方が入るようになり、重なる余地が
4時間ぶんに伸びた。取得元への要求は
`system_policy:public-web-low-frequency-v1` の下にあり、一日一度という境界は
待ち終わった側でも確かめる必要がある（`AGENTS.md` の外部取得の条件）。

- `monitor-recheck-due` を新設し、concurrency の待ちが明けた後、
  **いま在る状態で**もう一度 due を判定する。判定は plan と同じ `_is_due` を通す
  ので、大引けの規則も発表日の窓も一箇所のままである。
- 束を作る側の手順（run-monitor / upload / re-download / verify / handoff）を
  その結果で塞ぐ。通知や失敗報告の手順は `run-monitor` の結果を見ているので、
  塞がれた回は静かに何もしない。
- **手動の dispatch は通す。** plan を `--force` で抜けているのだから、
  ここでも抜ける。人が意図して呼んだ観測を、この門で止めない。
- **判定できないときは観測する。** 余分に一度取ることより、取り損ねることの方が
  害が大きい。失敗の向きを変えない。
...
### ERS-ADR-0082 Amendment (2026-09-06)

本ADRを書いた直後、同じゲートにもう一つ穴が見つかった。**指摘が無いとき、
Codexはレビューではなく普通のコメントで返す。**

PR #82 自身で実測した。

```
[13:15:08Z] chatgpt-codex-connector[bot]
Codex Review: Didn't find any major issues. Delightful!
Reviewed commit: c657ac4049
```

これは issue comment として投稿され、`pulls/82/reviews` には何も現れなかった。
ゲートはレビューだけを数えているので、**きれいなPRは必ず「レビュー未着」に
なり、fail-open で通る。**

通ること自体は正しい——未解決の指摘スレッドはゼロなのだから。しかし
**「Codexが止まっている」と「見て問題なかった」が区別できない。** fail-open は
前者のために置いた逃げ道で、後者に使われるとゲートは何も言っていないのと
同じになる。

どちらの形も本文に `Reviewed commit: <sha>` を持つ。現在の head を名指す
ボットのコメントも到着として数える。コメント側にも head の一致を要求し、
古い回の文言で即グリーンにしない。**成功条件は「未解決スレッドがゼロ」の
ままで、変えていない。**

テスト 1347→1349。

さらに Codex の P2 指摘。**呼べなかった回を、レビューが来なかった回と同じに
扱っていた。**

fork や Dependabot からのPRでは、`GITHUB_TOKEN` が job 単位の permissions に
よらず読み取り専用になる。依頼の POST は 403 で落ち、`continue-on-error` が
それを飲み込んで20分の fail-open へ流れる。**push では Codex は動かないので、
呼べていない回は誰も見ていない。** それを「Codexが止まっている」と同じ扱いで
通していた。

fail-open は Codex 側の停止のための逃げ道で、**こちらの手が届かなかった場合に
使うものではない。** 依頼の成否を待つ側へ渡し、呼べずにレビューも来なかった
回は赤にする。人が `@codex review` と書いて再実行すればよい、と同時に言う。

Codex は `pull_request_target` か専用の GitHub App を挙げている。どちらも
書き込み可能な経路を fork からのイベントで開くことになり、public repository では
入口が増える。**この repository に fork からのPRの運用は無い**（全コミットが
同一 repository の枝を経由する）ので、入口を開けるより、届かなかったことを
見えるようにして人に渡す方を採った。運用が変わればここを見直す。

テスト 1365→1367。

さらに Codex の P2 指摘。**重なった古い実行が、他人の head へレビューを頼む。**

続けて push すると `synchronize` の実行が重なる。古い方は自分の head SHA への
レビューを待つが、**Codexが見るのは常に最新の head なので、その待ちは満たされない。**
依頼のコメントは commit を名指さないので、古い実行が投げると最新の head が
レビューされ、投げた本人は自分の SHA を待ち続ける。20分の fail-open を消費し、
**限りある枠まで無駄に使う——この変更が節約しようとしているものである。**

このPR自身で条件が揃っていた。2026-09-07 の実行:

```
22:28 c98d816 （20分待ちの最中）
22:37 c112e80 ← 9分後に新しい head
```

- PR単位の `concurrency` を置き、`cancel-in-progress: true` で古い方を畳む。
  畳まれるのは古い commit の実行で、必須チェックが見るのは head の実行である。
- 畳まれる前に走り出した実行が残るので、**投げる直前に現在の head と突き合わせ、
  ずれていたら投げない。** 突き合わせは投稿より前に置く。

テスト 1367→1369。

さらに Codex の P1 指摘。**rebase の畳み方が、ADR の構造を壊していた。**

`## ERS-ADR-0082` の見出しが `## ERS-ADR-0081` の見出しと `Date:` の間に
割り込み、**0081 の全文が 0082 の中身として読まれる**状態になっていた。
0082 の本文は見出しを失い `Approval:` から始まっていた。**どちらの ADR も、
読む側からは別のものに見える。** 決定の記録としては、内容が正しくても意味がない。

衝突を「両方残す」で機械的に畳んだ結果である。同じ末尾に追記し続ける文書に
その畳み方を当てると、境界が混ざる。畳んだ後に構造を見ていなかった。

見出しと本文の並びをテストで固定した。前置きの形は歴史的にばらついている
（`Date:` 58本、`Title:` 20本、他3本）ので、**形は見ない。見るのは
「見出しの次に本文がある」ことと、番号が重複していないことだけ。**
衝突マーカーが残っていないことも併せて見る——一度そのまま commit した。

テスト 1394→1397。

さらに Codex の P2 指摘。**再実行のたびに頼み直し、レビューがマージの後に届く。**

待つ側は指摘が残ったまま時間切れになると「resolve して再実行せよ」と案内する。
**チェックの再実行は `synchronize` の event をそのまま持ち回る**ので、依頼の手順は
push 無しで何度でも走る。毎回頼むと、待つ側は既にある方のレビューを見てすぐ緑に
なり、**新しく頼んだレビューはマージの後に届く。**

ERS #48（マージ4分後にP1指摘が2件着き、「PRの修正」でなく「mainの負債」に
なった）と同じ形である。**このゲートはそれを防ぐために在る。** 直そうとした変更が
同じ穴を開けていた。

既に見てもらった SHA なら頼まない。数え方は待つ側とそろえ、**同じ二つの述語を
持っていることをテストで固定した。** 片方だけ変えると、頼むかどうかと通すかどうかが
食い違う。
...
**draft のうちも頼んでいた。** 書きかけへの push も `synchronize` なので、条件を
event だけにすると更新のたびに未完成の内容をレビューさせ、限りある枠を使い切る。
draft を見てもらう場所は `ready_for_review` で、そこは Codex 自身が発火する。
`github.event.pull_request.draft == false` を条件に足した。

**head の突き合わせが、投げる直前から離れていた。** 直前の修正で「既に見て
もらったか」の走査を head 確認と POST の間に入れたが、その走査はページを跨ぐので
時間がかかる。**その間に新しい push が入れば、古い実行が最新の head へ頼むという
一つ前に直したはずの競合がそのまま戻る。** 自分の修正が自分の修正を壊していた。

最初の確認は明らかに古い実行を早く畳むためのものとして残し、**投稿の直前でもう一度
突き合わせる。** 二度あることと、二度目が既レビュー判定より後にあることをテストで
固定した。
