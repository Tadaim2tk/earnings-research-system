...
**手で拾わせる選択肢は無い。** 毎日決まった時刻に決まった作業を人に渡すのは、自動化できるものを人の仕事にすることであって、この運用が避けたい形そのものである。

Decision:

- **`tools/capture_disclosures.py` を置き、日次で走らせる。** 既定は3日ぶん遡る——週末と、一度の失敗を吸収するため。
- **既定は台帳にある銘柄だけ。** `release.tdnet.info` の robots は `Disallow: /` で、索引が渡したURLに限った取得は ERS-ADR-0046 で Human が明示的に承認したが、**それは1社・四半期1本の文脈だった**。全短信は1日250本で30倍になる。規模の変更は人の判断なので、`--all` を明示しない限り広げない。既定で広がらないことをテストで固定した。
- **窓の外（404）と失敗を分けて数える。** 404 は取り返せないので記録するだけ、それ以外は原因が残る。混ぜると「毎日たくさん失敗している」ようにしか見えず、本物の障害が埋もれる。
- **保存するのは抽出テキストと `pdf_sha256` / `text_sha256` まで。** PDF本体は残さない（既存方針）。置き場はリポジトリの外——開示本文は第三者の内容であり、このリポジトリは public である。
- **採点はしない。** 確保と評価を同じ工程に入れると、`scoring_version` を変えるたびに再取得が要る。窓が31日しか無いので、それは取り返しがつかない。

Consequences: テスト 1201→1204。実測で、2026-08-13 は対象9件のうち8件が既存・1件を新規確保、2026-06-10 は対象4件すべてが窓の外と記録された。

**確保は前向きにしか効かない。** 今日以降に公開される短信は溜まるが、既に落ちたものは戻らない。数年分の過去ナラティブが要るなら EDINET（10年保存）が候補になるが、EDINET が持つのは有価証券報告書と半期報告書であって短信ではなく、**四半期の定性情報は短信にしか無い**。
