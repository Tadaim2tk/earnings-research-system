...
- **境界は「レジストリの凍結」ではなく「最初の trial の到着」に置く。** 何も観測していない定義は、観測結果に応じて調整されたはずがない。開始前は rule を変更して再freezeできる。最初の trial が入った瞬間に固定される。
...
- **`verify-stop-rule-tightening` を `verify-successor-registry` に改名する。** 判定基準が「厳格化されているか」ではなくなったので、名前が事実と食い違う。後継検査に残る中身は、レジストリ不一致・version逆行・仮説の脱落・**同一 hypothesis_version 下での rule 変更**である。

Consequences: テストは 1051→1064。CIに `verify-rule-freeze` を追加した。committed trial が0件なので**今日この経路は空の記録に対して通る**。これは意図した状態であり、能力そのものは synthetic registry に対して実際に発火させて確認している。
