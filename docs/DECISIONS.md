...
この検査自身も最初は `###` で始まる見出しだけを見ており、docstring は「renderer が実際に出すもの」と書いていた。**規約1の違反を、規約1を実装するコードが犯していた**。dashboard は `## 仮説検証` や `## 直近の記録` で既に `##` を使っているので、新しいコホート表を `##` で足す書き方は十分あり得る。見出しの深さではなく**その下に公開図表の書式のセルが並んでいるか**で判定する形に変え、`#` から `####` まで全レベルで落ちることを実測した。件数だけの「手動レビュー結果の集計」は図表の書式を持たないので誤検出しない。
