...
`outputs/historical_research` holds the migration's committed views: dashboard, weekly report, note draft, publishing parity, and aggregation summary. **They are v1 and predate the statistical guards** — no reserved period, returns measured from the previous close, no multiplicity correction — so their figures record what the retired system published rather than findings, and the gap cohorts in them reverse sign once measured from a tradeable price. The six Markdown files say so at the top; `aggregation_summary.json` carries a `superseded_note` key, and `research_knowledge.json` and `publishing_parity.json` carry nothing — the first is hash-bound to the frozen hypothesis registry and cannot be annotated without breaking verification. The same directory also holds four files `knowledge.py` still generates (`research_knowledge.json`, `research_report.md`, and two digests), which have none of the guards. `outputs/historical_research/SUPERSEDED.md` explains both origins. The current pipeline reaches no claim that survives correction; see ERS-ADR-0045. The old daily AI selection and yfinance enrichment are not part of ERS.
